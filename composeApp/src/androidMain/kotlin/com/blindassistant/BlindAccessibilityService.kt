...
class BlindAccessibilityService : AccessibilityService() {
    ...
    companion object {
        ...
        fun cleanYouTubeVideoTitle(raw: String): String {
            val lowerCheck = raw.trim().lowercase()
            if (lowerCheck.startsWith("did you mean") || lowerCheck.contains("did you mean") ||
                lowerCheck.startsWith("showing results for") || lowerCheck.contains("showing results for") ||
                lowerCheck.startsWith("search instead for") || lowerCheck.contains("search instead for") ||
                lowerCheck.startsWith("including results for") || lowerCheck.contains("including results for") ||
                lowerCheck.startsWith("searches related to") || lowerCheck.contains("searches related to")) {
                return ""
            }
        ...
        fun isUiNoiseOrStatusTitle(raw: String): Boolean {
            ...
            if (lower.startsWith("did you mean") || lower.contains("did you mean") ||
                lower.startsWith("showing results for") || lower.contains("showing results for") ||
                lower.startsWith("search instead for") || lower.contains("search instead for") ||
                lower.startsWith("including results for") || lower.contains("including results for") ||
                lower.startsWith("searches related to") || lower.contains("searches related to") ||
                lower.startsWith("try searching for") || lower.contains("try searching for")) {
                return true
            }
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        if (event.eventType == AccessibilityEvent.TYPE_NOTIFICATION_STATE_CHANGED) {
            ...
            if (isWhatsAppPackage(pkg) || pkg.contains("messaging") || pkg.contains("sms") || pkg.contains("mms")) {
                handleIncomingMessageNotification(pkg, title, body, rawTexts, notification)
            }
        }
    }
    ...
    private fun handleIncomingMessageNotification(
        ...
        // 1. Extract real message lines from EXTRA_TEXT_LINES or EXTRA_MESSAGES
        val lines = notification?.extras?.getCharSequenceArray(Notification.EXTRA_TEXT_LINES)
        val messageLines = lines?.mapNotNull { it?.toString()?.trim() }?.filter { it.isNotBlank() }

        val messagingParcelables = notification?.extras?.getParcelableArray("android.messages")
        val messagingTexts = messagingParcelables?.mapNotNull {
            (it as? Bundle)?.getCharSequence("text")?.toString()?.trim()
        }?.filter { it.isNotBlank() }

        var messageContent = when {
            !messageLines.isNullOrEmpty() -> messageLines.joinToString(". ")
            !messagingTexts.isNullOrEmpty() -> messagingTexts.joinToString(". ")
            else -> cleanBody
        }

        // Filter out system / noise / status notifications
        val lowerBody = messageContent.lowercase()
        ...
        if (cleanTitle.equals("WhatsApp", ignoreCase = true) || cleanTitle.equals("Messages", ignoreCase = true) || cleanTitle.isBlank()) {
            if (messageText.contains(": ")) {
                sender = messageText.substringBefore(": ").trim()
                messageText = messageText.substringAfter(": ").trim()
            } else {
                sender = "Someone"
            }
        } else if (messageText.startsWith("$cleanTitle: ", ignoreCase = true)) {
            messageText = messageText.removePrefix("$cleanTitle: ").removePrefix("$cleanTitle:").trim()
        }

        // Handle generic notification count summaries (e.g. "2 new messages from Ali")
        val summaryRegex = Regex("""(?i)^\d+\s+(?:new\s+)?messages?(?:\s+from\s+(.*))?""")
        val summaryMatch = summaryRegex.find(messageText)
        if (summaryMatch != null) {
            val extractedSender = summaryMatch.groupValues.getOrNull(1)?.trim()
            if (!extractedSender.isNullOrBlank()) {
                sender = extractedSender
            }
            if (!messageLines.isNullOrEmpty()) {
                messageText = messageLines.joinToString(". ")
            }
        }

        val isVoiceNote = messageText.contains("Voice message", ignoreCase = true) ||
                messageText.contains("voice note", ignoreCase = true) ||
                messageText.contains("Audio", ignoreCase = true) ||
                messageText.contains("PTT", ignoreCase = true) ||
                messageText.contains("🎙️") ||
                cleanBody.contains("Voice message", ignoreCase = true) ||
                cleanBody.contains("🎙️") ||
                fallbackText.contains("Voice message", ignoreCase = true)
        ...
        val announcement = if (isVoiceNote) {
            "New voice message from $sender. Say play voice note to listen."
        } else {
    ...
    private fun findVoiceNotePlayButton(node: AccessibilityNodeInfo): AccessibilityNodeInfo? {
        ...
        if (desc.contains("play voice message") || desc.contains("play audio") || desc.contains("play button") ||
            desc == "play" || desc.startsWith("play ") || desc.contains("reproducir") ||
            viewId.contains("play") || viewId.contains("voice_message") || viewId.contains("audio") ||
            viewId.contains("conversation_row_button")) {
            return node
        }
    ...
    fun isUiNoiseOrStatusTitle(raw: String): Boolean = Companion.isUiNoiseOrStatusTitle(raw)
    ...
    private fun isNonVideoCandidate(c: YouTubeVideoCandidate): Boolean {
        ...
        val tLower = c.title.lowercase()
        if (tLower.contains("did you mean") || tLower.contains("showing results for") || tLower.contains("search instead for")) return true
        val src = c.sourceNode
